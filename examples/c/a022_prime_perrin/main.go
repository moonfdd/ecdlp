...
func Perrin(n *big.Int) bool {
	...
	if false {
		ll := LucasParam{big.NewInt(2), big.NewInt(-1)}
	...
	if true {
		...
		b[1][0] = big.NewInt(0)
		b[2][0] = big.NewInt(2)
		juZhenMul := JuZhenMul(juZhenPow, b, n)
		r := juZhenMul[0][0].Cmp(big.NewInt(0)) == 0
		if !r {
			return r
		}
	}
	...
	if true {
		...
		juZhenMul := JuZhenMul(juZhenPow, b, n)
		p_2 := juZhenMul[0][0]
		p_2.Add(p_2, big.NewInt(1)).Mod(p_2, n)
		return p_2.Cmp(big.NewInt(0)) == 0
		// r := juZhenMul[0][0].Cmp(big.NewInt(0)) == 0
		// if !r {
		// 	return r
		// }
...
func JuZhenMul(a, b [][]*big.Int, n *big.Int) [][]*big.Int {
	// fmt.Println(len(a), len(a[0]), len(b), len(b[0]))
	ans := make([][]*big.Int, len(a))
	for i := 0; i < len(a); i++ {
		ans[i] = make([]*big.Int, len(b[i]))
		for j := 0; j < len(b[i]); j++ {
			ans[i][j] = big.NewInt(0)
		}
	}
	// fmt.Println("ans = ", len(ans), len(ans[0]))

	for i := 0; i < len(a); i++ {
		for j := 0; j < len(a[i]); j++ {
			for k := 0; k < len(b[j]); k++ {
				ans[i][k].Add(ans[i][k], big.NewInt(0).Mul(a[i][j], b[j][k]))
				if n != nil {
					ans[i][k].Mod(ans[i][k], n)
				}
			}
		}
	}
...
func JuZhenPow(a [][]*big.Int, b *big.Int, n *big.Int) [][]*big.Int {
	ans := make([][]*big.Int, len(a))
	b = big.NewInt(0).Set(b)
	for i := 0; i < len(a); i++ {
		ans[i] = make([]*big.Int, len(a[i]))
		for j := 0; j < len(a[i]); j++ {
			if i == j {
				ans[i][j] = big.NewInt(1)
			} else {
				ans[i][j] = big.NewInt(0)
			}
		}
...
// func JuZhenMul(n *big.Int) []*big.Int {
// 	base := make([][]*big.Int, 3)
// 	for i := 0; i < 3; i++ {
// 		base[i] = make([]*big.Int, 3)
// 	}
// 	base[0][0] = big.NewInt(0)
// 	base[0][1] = big.NewInt(1)
// 	base[0][2] = big.NewInt(0)
// 	base[1][0] = big.NewInt(0)
// 	base[1][1] = big.NewInt(0)
// 	base[1][2] = big.NewInt(1)
// 	base[2][0] = big.NewInt(1)
// 	base[2][1] = big.NewInt(1)
// 	base[2][2] = big.NewInt(0)

// }

// https://en.wikipedia.org/wiki/Perrin_number#Perrin_pseudoprimes  矩阵运算
func main() {
	if true {
		if true {
			fmt.Println("开始测试")
			...
			for ; num.Cmp(rightLimit) <= 0; num.Add(num, big.NewInt(2)) {
