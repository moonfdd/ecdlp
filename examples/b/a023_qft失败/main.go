...
func Qft(n *big.Int) bool {
	...
	b := big.NewInt(0)
	for {
		b4c := big.NewInt(0).Mul(b, b)
		// q := big.NewInt(0).Sub(b4c, big.NewInt(0).Mul(c, big.NewInt(4)))
		// if q.Cmp(big.NewInt(0)) == 0 {
		// 	b.Add(b, big.NewInt(1))
		// 	continue
		// }
		b4c.Add(b4c, big.NewInt(0).Mul(c, big.NewInt(4)))
		j := Jacobi(b4c, n)
		if j.Cmp(big.NewInt(0)) == 0 {
			return n.ProbablyPrime(0)
			fmt.Println("b = ", n, n, "合数")
			return false
		}
		if j.Cmp(big.NewInt(-1)) == 0 {
			break
		...
		b.Add(b, big.NewInt(1))
		// fmt.Println("b temp", b, n)
	}
	// fmt.Println("b", b, n)
	oper := NewPolynomialOperation(big.NewInt(0), n)
	oper.Xr = []*big.Int{big.NewInt(1), big.NewInt(0).Neg(b), big.NewInt(0).Neg(c)}
	fmt.Println(oper.Xr, n)
	nplus1 := big.NewInt(0).Add(n, big.NewInt(1))
	pow := oper.PowerMod([]*big.Int{big.NewInt(1), big.NewInt(0)}, big.NewInt(0).Div(nplus1, big.NewInt(2)))
	fmt.Println("pow1 = ", pow, n)
	pow = oper.PowerMod([]*big.Int{big.NewInt(1), big.NewInt(0)}, nplus1)
	fmt.Println("pow2 = ", pow, n)

	s := big.NewInt(0).Mul(n, n)
	...
	pow = oper.PowerMod([]*big.Int{big.NewInt(1), big.NewInt(0)}, s)
	fmt.Println("pow3- = ", pow, n)
	if len(pow) == 1 && ( /*pow[0].Cmp(big.NewInt(1)) == 0 ||*/ pow[0].Cmp(big.NewInt(0).Sub(n, big.NewInt(1))) == 0) {
		fmt.Println("素数1", n)
		return true
	}
	for i := 0; i <= r-2; i++ {
		pow2 := oper.PowerMod(pow, big.NewInt(2))
		fmt.Println("pow3 = ", pow2, n)
		if len(pow2) == 1 && ( /*pow2[0].Cmp(big.NewInt(1)) == 0 ||*/ pow2[0].Cmp(big.NewInt(0).Sub(n, big.NewInt(1))) == 0) {
			fmt.Println("素数2", n)
			return true
		}
		pow = pow2
	}
	// fmt.Println("pow4 = ", pow, n)
	return false
}
